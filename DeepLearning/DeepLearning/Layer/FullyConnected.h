...
private:
	...
	Matrix m_din;
public:
	FullyConnected(const int ins_size, const int out_size):
		Layer(in_size,out_size)
	{}
	void init(const Scalar& mu, const Scalar& sigma, RNG& rng) {
		m_weight.resize(this->m_in_size);
		...
		{
			//z=w' * in +b
			const int nobs = prev_layer_data;
			m_z.resize(this->m_out_size, nobs);
			m_z.noalias() = m_weight.transponse() * prev_layer_data;
			m_z.colwise() += bias;

			m_a.resize(this->m_out_size, nobs);
			Activatio::activate(m_z,m_a);
		...
		std::vector<Scalar> get_derivatives() const = 0;
	}
